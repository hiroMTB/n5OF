...
void ofApp::draw_info() {
	...
	case 1: 
		ss << "OSC, port " << OSC_PORT << ", frame offset " << sync_offset_frame << ", ";
	...
	sprintf(mm, "%-4s  %-20s  %-8s  %-8s  %-8s  %-8s  %-8s  %-8s  %-15s",
		"Axis", "name", "offset(mm)", "tarpos", "actpos", "ErrorID", "Disabled", "Homed", "motion state" );
	ss << mm << "\n";

	for (int i = 0; i<axis.size(); i++) {
		Axis &a = axis[i];
		sprintf(mm, "%4d  %20s  %8.2f  %8.2f  %8.2f  %8d  %8d  %8d  %15s",
					i+1, a.name.c_str(), a.cmd_offset, a.tarpos, a.actpos, a.ErrorID, a.Disabled, a.Homed, Axis::MC_MotionSate[a.motion_state]);
		ss << mm << "\n";
	}
...
void ofApp::keyPressed(int key) {
	switch (key) {
		...
		case 'd':
			for (int i = 0; i < 10; i++) {
				if (axis[i].Homed && axis[i].ErrorID==0) {
