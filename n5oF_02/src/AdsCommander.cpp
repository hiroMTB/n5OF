...
void AdsCommander::goTo( string name, float pos, float vel) {
	ads.writeValue<double>(name + ".cmd_pos", pos);
	ads.writeValue<double>(name + ".cmd_vel", vel);
	//ads.writeValue<double>(name + ".cmd_acc", acc);
	//ads.writeValue<double>(name + ".cmd_deacc", deacc);
	//ads.writeValue<double>(name + ".cmd_jerk", jerk);
...
void AdsCommander::goToAll(float pos, float vel) {
	for (int i = 0; i<10; i++) {
